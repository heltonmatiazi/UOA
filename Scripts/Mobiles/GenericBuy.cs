...
namespace Server.Mobiles
{
	public class GenericBuyInfo : IBuyItemInfo
	{
		...
		public static int GetRandBuyPriceFor( int itemPrice )
		{
			...
			double lowPrice = .5;
			double highPrice = 1.5;
		
			if (AetherGlobe.DoomCurse < 20000)
			{
				lowPrice = ( .75 * (double)itemPrice );
				highPrice = ( 1.1 * (double)itemPrice );
			}	
			else if (AetherGlobe.DoomCurse < 60000)
			{
				lowPrice = ( .95 * (double)itemPrice );
				highPrice = ( 1.3 * (double)itemPrice );
			}		
			else if (AetherGlobe.DoomCurse > 60000)
			{
				lowPrice = ( 1.3 * (double)itemPrice );
				highPrice = ( 1.7 * (double)itemPrice );
			}				
			
			double price = 0;
			for ( int i = 1; i <= 3; i++ )
			{
				price += (double)Utility.RandomMinMax( (int)lowPrice, (int)highPrice );
			}
			price /= 3;
			if ( price < 1 )
				price = 1;
		...
		public static int GetRandBuyAmountFor( int itemAmount )
		{
			...
			int lowAmount = (int) .5;
			int highAmount = (int)1.5;
			
			if (AetherGlobe.DoomCurse < 2000)
			{
			lowAmount = (int)( .4 * itemAmount );
			highAmount = (int)( 1.1 * itemAmount );
			}	
			else if (AetherGlobe.DoomCurse < 6000)
			{
			lowAmount = (int)( .8 * itemAmount );
			highAmount = (int)( 1.3 * itemAmount );
			}		
			else if (AetherGlobe.DoomCurse > 6000)
			{
			lowAmount = (int)( 1 * itemAmount );
			highAmount = (int)( 1.7 * itemAmount );
			}	
