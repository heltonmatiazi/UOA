...
namespace Server.Mobiles
{
	...
	public class BaseCreature : Mobile, IHonorTarget
	{
		...
		public virtual int BreathComputeDamage()
		{
			...
			if ( damage > 350 ){ damage = 350; } // final making breath damage deal more damage
        ...
        {
            if (killer is PlayerMobile || this is HenchmanArcher || this is HenchmanFighter || this is HenchmanMonster || this is HenchmanWizard || this is Squire || !this.JakoIsEnabled || (killer is BaseCreature && ((BaseCreature)killer).Controlled && ((BaseCreature)killer).ControlMaster != null || !((BaseCreature)killer).JakoIsEnabled))
                return;
        ...
        public uint setLevel(uint newLevel, bool tellOwner)
        {
            ...
            if (newLevel < m_level)
            {
				...
				if (m_level >= 25 && newLevel < 25)
					this.ControlSlots += 1;

				if (m_level >= 50 && newLevel < 50)
					this.ControlSlots += 1;
            ...
            else if (newLevel > m_realLevel)
            {
				...
				if (m_realLevel < 50 && newLevel >= 50 && this.ControlSlots >= 1)
					this.ControlSlots -= 1;
		...
		*/
		...
		public static bool AlwaysInvulnerable( Mobile m )
		{
			...
			//else if ( m is CharacterClone ){ return true; }
		...
		public override void OnAfterSpawn()
		{
			...
			if ( (reg.IsPartOf( "DarkMoor" ) || reg.IsPartOf( "the Temple of Praetoria" )) && !(this is BaseCursed) && !(this is BaseVendor) && !(this is Citizens) && !(this is BasePerson) && !(this is BaseRed) && !(this is BaseQuester) && this.Karma < -50)
			...
			if (Utility.RandomDouble() < (AetherGlobe.DoomCurse / 150000) && Utility.RandomBool() && this.Karma < 0 && !this.Tamable && !(this is BaseVendor) && !(this is BaseCursed) && !(this is BasePerson) && !(this is BaseRed) ) // FINAL evil curse bump in stats
			{
				this.RawStr *= 2;
				this.RawDex *= 2;
				this.RawInt *= 2;
				...
				DynamicFameKarma();
				if ( Utility.RandomDouble() <  ( 0.10 *  ((double)Math.Abs(this.Karma) / 20000) ) )
					this.PackItem ( new BalanceSpike() );
			}
			else if (Utility.RandomDouble() < ( (100000-AetherGlobe.DoomCurse) / 150000) && Utility.RandomBool() && this.Karma > 0 && !this.Tamable && !(this is BaseVendor) && !(this is BasePerson) && !(this is BaseBlue) && !(this is BaseChild))  // FINAL evil curse bump in stats
			{
				this.RawStr *= 2;
				this.RawDex *= 2;
				this.RawInt *= 2;
			...
			if (this.Map == Map.Ilshenar && this.X <= 1007 && this.Y <= 1280 && ( this is BasePerson || this is BaseGuildmaster || this is TownHerald || this is BaseVendor || this is AnimalTrainerLord || this is Townsperson || this is PlayerVendor || this.Blessed || this.FightMode == FightMode.None || this is Citizens) )
			...
			else if ( !(this is BasePerson) && !(this is BaseVendor) && !(this is AnimalTrainerLord) && !(this is Townsperson) && !(this is PlayerVendor) && !(this.Blessed) && this.FightMode != FightMode.None && !(this is Citizens) )	
				{
					...
					if (this.AI == AIType.AI_Mage || this.AI == AIType.AI_NecroMage || this.AI == AIType.AI_Mage2 )
						modifier= 1 + 0.20 + (( this.Skills[SkillName.Magery].Value + this.Skills[SkillName.EvalInt].Value ) / 200.0);
					else if  (this.AI == AIType.AI_Archer || this.AI == AIType.AI_Archer2)
						modifier= 1 + (( this.Skills[SkillName.Archery].Value + this.Skills[SkillName.Tactics].Value ) / 200.0);
					else 
					...
					if (this.HasBreath)
						modifier += (double)this.BreathComputeDamage() / 350.0;
					...
					if ( this is BaseBlue || this is BaseRed ) //harder mob types
						modifier *=1.5;
					...
					double resistances = ( ((double)this.PoisonResistSeed + (double)this.PhysicalResistanceSeed + (double)this.FireResistSeed + (double)this.ColdResistSeed + (double)this.EnergyResistSeed) / 400.0 );
					double finalcompute =  ( ( ( ( (this.RawStr + this.RawDex + this.RawInt + this.HitsMax) * this.DamageMax) / 10 ) * resistances) * modifier * 5 ); 

					//diminishing returns to prevent -60,000 karma mobs
					double final = 0;

					if (finalcompute < 1000)
						final = finalcompute;				
					else 
					{	
						while ( finalcompute > 0 )
						{
							if (finalcompute > 1000)
							{
								...
								if (final > 20000)
									finalcompute *= (0.9 * ((40000 - final) / 20000)); // limits to 35k karma
								else
									finalcompute *= 0.90;
							}
							else
							{
								final += finalcompute * .90;
								finalcompute = 0;
							}
						}
					}
			...
			if ( this.Tamable && !(this.Controlled)  )	
				{
					...
					if ( this is BaseBlue || this is BaseRed ) //may be tamable in the future
						modifier *=1.5;
					
					if ( this is DemonKnight || this is BaseChampion) //may be tamable in the future
						modifier *= 2;
					...
					if (this.ControlSlots > 1)
						finalcompute /= ( 1 + ((this.ControlSlots+2)/10) );

					//diminishing returns to prevent 125+ min taming required
					double final = 0;

					double karmatotame = 45; // tuned to be 125 at 30k fame (no tamables would have that much under dynamicfamekarma)
					double diminish = 0.70; // 45/60/500 was 65
					double step = 500;
					...
					{	
						while ( finalcompute > 0 )
						{
							if (finalcompute > step)
							{
								finalcompute -= step;
								//if (final >100)
									//final += (step / karmatotame) / (1+ ((final - 55)/100)) ;
								//else 
								...
								if (final < 90)
									...
									finalcompute *= (diminish * ((124 - final) / 34)); // makes sure max is 124 skill
							}
							else
							{
					...
					if ( amount > 0 )
					{
						...
						//{
							for ( int i = 0; i < amount; ++i )
							{
								if ( m_Loyalty < MaxLoyalty  )
								{
									m_Loyalty += Utility.RandomMinMax(5, 10);
								}
							}
		...
		public virtual bool Teach( SkillName skill, Mobile m, int maxPointsToLearn, bool doTeach )
		{
			...
			switch ( res )
			{
				...
				{
					m.SendMessage( "Make sure this skill is marked to raise. If you are near the skill cap (1,000 points) you may need to lose some points in another skill first.");
					break;
				}
...
//Final: standardize lootpack				
			...
			if (pack == LootPack.Poor || pack == LootPack.Meager || pack == LootPack.Average || pack == LootPack.Rich || pack == LootPack.FilthyRich || pack == LootPack.UltraRich || pack == LootPack.SuperBoss || pack == LootPack.AosPoor || pack == LootPack.AosMeager || pack == LootPack.AosAverage || pack == LootPack.AosRich || pack == LootPack.AosFilthyRich || pack == LootPack.AosUltraRich || pack == LootPack.AosSuperBoss || pack == LootPack.SePoor || pack == LootPack.SeMeager || pack == LootPack.SeAverage || pack == LootPack.SeRich || pack == LootPack.SeFilthyRich || pack == LootPack.SeUltraRich || pack == LootPack.SeSuperBoss )
			{
				DynamicFameKarma();
				...
				{
					pack = LootPack.Poor;
					if (Utility.RandomDouble() < (this.Fame / 500) )
						amount = 2;
					else
						amount = 1;
				}
					
				else if (this.Fame <= 4000)
				{
					pack = LootPack.Meager;
					if (Utility.RandomDouble() < (this.Fame / 4000) )
						amount = 2;
					else
						amount = 1;
				}
				else if (this.Fame <= 10000)
				{
					...
					if (Utility.RandomDouble() < ((this.Fame / 10000) /2) )
						amount = 2;
					else
						amount = 1;
				}
				else if (this.Fame <= 16000)
				{
					pack = LootPack.Rich;
					if (Utility.RandomDouble() < ((this.Fame / 16000)/3) )
						amount = 2;
					else
						amount = 1;
				}
				else if (this.Fame <= 22000)
				{
					pack = LootPack.FilthyRich;
					if (Utility.RandomDouble() < ((this.Fame / 22000)/3.5) )
						amount = 2;
					else
						amount = 1;
				}
				else if (this.Fame <= 30000)
				{
					...
					if (Utility.RandomDouble() < ((this.Fame / 30000)/3.5) )
						amount = 2;
					else
						amount = 1;
				}		
				else if (this.Fame > 30000)
				{
					...
					if (Utility.RandomDouble() < ((this.Fame / 30000)/4) )
						amount = 2;
					else
						amount = 1;
				}		
		...
		public override void AddNameProperties( ObjectPropertyList list )
		{
			...
			if ( this is SythProjection || this is Clown || this is Clone ){} // NO WORDS
		...
		public override bool OnBeforeDeath()
		{
           ...
           //Start Zombiex edit
            if (LastKiller is Zombiex || (LastKiller is BaseCreature && ((BaseCreature)LastKiller).CanInfect))
            {
...
// final - Doom drops
			if (!NoKillAwards && ( Region.IsPartOf("Doom") || Region.IsPartOf("Doom Gauntlet") ) )
			{
				...
				if (Utility.Random(dropchance) == 0)
				{
...
//end
		...
		public override void OnDeath( Container c )
		{
			...
			else
			{
				...
				//if ( ( killer.Karma <= -12500 ) && killer is PlayerMobile ) // anything a really evil player kills adds to the curse
				//	this.Karma = Math.Abs(this.Karma);
				
/*				if (this is DemonKnight) // killing the dark father really affects the curse.
				{
					if (killer is PlayerMobile)
						((PlayerMobile)killer).BalanceEffect += -300;
					AetherGlobe.ChangeCurse( -300 );	
				}*/
				if (this is BaseChild)
				{
					if (killer is PlayerMobile)
						((PlayerMobile)killer).BalanceEffect += 50;
					AetherGlobe.ChangeCurse( 50 ); // killing children adds to the curse
				}
				else 
				{
					...
					if (killer is BaseCreature)
					{
						if ( ((BaseCreature)killer).Controlled && ((BaseCreature)killer).ControlMaster != null && ((BaseCreature)killer).ControlMaster is PlayerMobile )
							killer = ((BaseCreature)killer).ControlMaster; // tamed pet killed this foe
					...
					if (killer is PlayerMobile)
					{
						...
						double changechange = ((double)this.Karma / 1000) ;//* ((double)Math.Abs(killer.Karma) / 10000);
						...
						if ( killer.Karma < 0)
						{
							karmaeffectneg = (double)Math.Abs(killer.Karma) / 10000;
							if (karmaeffectneg >= 1)
								karmaeffectpos = 0;
							else
						...
						{
							karmaeffectpos = (double)killer.Karma / 15000;
							karmaeffectneg = 0;
						...
						if ( !((PlayerMobile)killer).NormalMode )
							changechange /= 2;
						
						if (killer.Kills >0 && changechange >0)
							changechange *= 1+ (killer.Kills / 100);
						
						if (Math.Abs(changechange) < 1 && Utility.RandomDouble() < 0.33 )
						{
							if (changechange < 0)
								changechange = -1;
							else 
						...
						((PlayerMobile)killer).BalanceEffect += (int)changechange;
			...
			foreach ( Mobile m in World.Mobiles.Values )
			{
				...
				if ( m is BaseCreature )
				{
					...
					else if ( c.Controlled && c.Commandable && !c.Summoned && !c.IsDeadBondedPet && !c.IsStabled) 
                    {
						...
						{
							if (c.IsHitchStabled && Utility.RandomDouble() > 0.95)
								c.Loyalty -= 1;
							else if (Utility.RandomDouble() > 0.65)
