...
/*		public DateTime m_NextResurrect;
		...
		public override bool IsEnemy( Mobile m )
		{
		    ...
		    if ( m is PlayerMobile && m.Criminal || m.Karma <= -500 )
				return false;
		...
		{
			if ( !m.Frozen && ((BaseCursed)this).res && InRange( m, 4 ) && !InRange( oldLocation, 4 ) && InLOS( m ) && m is PlayerMobile )
			{
...
*/
        public void OneTimeTick()
        {
			...
			if (((BaseCursed)this).drainstart && this.Combatant != null)
			{
				...
				if (this.Hits <= this.HitsMax && this.CanSee( target ) && target.Hits > 0) // link is active
				{
					((BaseCursed)this).absorb +=1;
									
					this.Hits += ((BaseCursed)this).absorb * 5; // transfer of health
					target.Hits -= ((BaseCursed)this).absorb * 5;
        ...
        public static void NoTames( BaseCreature pet, Mobile target )
		{
			if ( pet == null || target == null || Utility.RandomDouble() < 0.33  )
				return;

            else if ( pet.Controlled && pet.CanBeHarmful(target))
            {
