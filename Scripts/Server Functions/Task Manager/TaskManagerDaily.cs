...
// Checking on investments	
				...
				for ( int i = 0; i < players.Count; ++i )
				{
						...
						m.BalanceChampion = false;
							
						if (m.BalanceEffect < 0 && m.NormalMode ) //contributed to good
						{
							if (goodchamp == null && m.Kills == 0)
								goodchamp = m;
							else if (m.BalanceEffect < goodchamp.BalanceEffect && m.Kills == 0)
								goodchamp = m;
						}
						else if (m.BalanceEffect > 0 && m.NormalMode ) //contributed to evil
						{
							if (evilchamp == null)
								evilchamp = m;
				...
				if (goodchamp != null)
				{
					goodchamp.BalanceChampion = true;
					AetherGlobe.GoodChamp = (Mobile)goodchamp;
				}
				if (evilchamp != null)
				{
					evilchamp.BalanceChampion = true;
