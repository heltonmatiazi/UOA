    ...
    public static class MaxFollowersIntelBased
    {
        ...
        public static void Evaluate(Mobile m)
        {
            if (Config.MaxFollowersAllowed - Config.MinFollowersAllowed > 0)
            {
				...
				else if ( m.Skills[SkillName.Herding].Base >= 100 )
					m.FollowersMax += 1;

				else if ( m.Skills[SkillName.Herding].Base >= 80 )
					m.FollowersMax += 1;
				...
				else if ( (m.Skills[SkillName.Veterinary].Base >= 80) && (m.Skills[SkillName.AnimalLore].Base >= 80) && (m.Skills[SkillName.AnimalTaming].Base >= 80) )
						m.FollowersMax += 1;					
