...
namespace Server.Misc
{
	public class SkillCheck
	{
		...
		public static bool CheckSkill( Mobile from, Skill skill, object amObj, double chance )
		{
			...
			if (from is PlayerMobile && from.Hunger <= 3 || from.Thirst <= 3)
				gc /= 2.5;
			else if (from is PlayerMobile && from.Hunger <= 10 || from.Thirst <= 10)
				gc /= 1.25;
			else if (from is PlayerMobile && from.Hunger >= 18 || from.Thirst >= 18)
				gc *= 1.15;
