...
namespace Server.Gumps
{
	public class ResurrectCostGump : Gump
	{
		...
		public ResurrectCostGump( Mobile owner, int healer ) : base( 150, 50 )
		{
			...
			if (owner.Karma >= 0)
				penalty = ((double)AetherGlobe.DoomCurse / 100000.0) * ( ((double)karma / 15000));
			else
				penalty =  ( (100000-(double)AetherGlobe.DoomCurse) / 100000.0) * ( ((double)karma / 15000));
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if( info.ButtonID == 1 )
			{
				...
				if ( m_ResurrectType == 2  )
				{
					...
					{
						if ( !((PlayerMobile)from).BalanceChampion )
						{
