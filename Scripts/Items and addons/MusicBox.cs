...
namespace Server.Items
{
	public class MusicBox : Item
	{
		...
		{
			if ( Mplay == 1){ m.Send(PlayMusic.GetInstance( MusicName.OldUlt01 )); m.SendMessage( "OldUlt01" ); Mplay = Mplay + 1; }
			else if ( Mplay == 2){ m.Send(PlayMusic.GetInstance( MusicName.Create1 )); m.SendMessage( "Create1" ); Mplay = Mplay + 1; }
